...
const App = () => {
  ...
  const handleBacktranslate = async () => {
    ...
    /* 
      Generic language mapping for translation services.
    */
  ...
  const handleEvaluate = async () => {
    try {
      ...
      showError(`An error occurred evaluating the translation. Please try again.\n${e}`)
      setIsEvaluating.off()
      setEvaluationResult([])
    }
  }
      ...
      {(translation.length < 1)
        ? <>
          ...
          <HStack align='center' alignSelf='center'>
            ...
            <Tooltip label='Translations are performed using local models (Argos Translate).'><InfoOutlineIcon ml="0.25rem" mb="0.25rem" /></Tooltip>
          </HStack>
        </>
        : <> 
          {errorMessage && <Alert status='warning' mb="1rem" variant="left-accent"><AlertIcon />{errorMessage}</Alert>}
